...
export default {
    ...
    computed: {
        ...
        sliderMin: function() {
            ...
            var heatMapValues = this.dataHeatMapPiling[0]["src"]["data"].filter(
                element => element
            );
    ...
    methods: {
        initPiling: function(domElementID, renderer, data) {
            this.piling = createLibrary(document.getElementById(domElementID), {
                renderer: renderer,
                items: data,
                itemSize: this.width,
                previewPadding: 0,
