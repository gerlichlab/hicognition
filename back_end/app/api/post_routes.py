...
def preprocess_dataset():
    ...
    if (Dataset.query.get(dataset_id).filetype == "cooler") and is_access_to_dataset_denied(Dataset.query.get(dataset_id), g.current_user):
        return forbidden(f"Cooler dataset is not owned by logged in user!")
    if (Dataset.query.get(dataset_id).filetype == "bigwig") and is_access_to_dataset_denied(Dataset.query.get(dataset_id), g.current_user):
