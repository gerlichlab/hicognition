...
class TestPreprocessCollections(LoginTestCase, TempDirTestCase):
    ...
    def setUp(self):
        ...
        # create collections
        collection = Collection(datasets=[dataset1, dataset2], user_id=1, id=1)
        collection2 = Collection(datasets=[dataset3], user_id=2, id=2)
        db.session.add_all(
            [dataset1, dataset2, dataset3, dataset4, interval1, collection, collection2]
        )
        db.session.commit()
    ...
    def test_tasks_deleted_after_relaunch(self, mock_get_rq_job, mock_launch):
        ...
        task2 = Task(id="test2", name="test2", user_id=1, collection_id=1, complete=False)
