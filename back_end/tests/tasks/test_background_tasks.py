...
from app.background_tasks import add_app_context, cleanup_empty_tasks, cleanup_failed_tasks
...
class TestCleanupFailedTasks(LoginTestCase):
    ...
    def setUp(self):
        ...
        self.failed_task_no_associations = Task(
            id="wJob3"
            
        )
        self.failed_task_dataset = Task(
            id="wJob4",
            dataset_id=2,
            intervals_id=1
        )
        self.failed_task_collection = Task(
            id="wJob4",
            collection_id=1,
            intervals_id=1
        )
    ...
    def test_bad_tasks_removed(self, mock_set_collection_failed, mock_set_dataset_failed):
        """Tests whether tasks that failed are deleted"""
        db.session.add_all([self.detached_task, self.completed_task, self.failed_task_no_associations])
        db.session.commit()
        # disptach call
        add_app_context(self.app)(cleanup_failed_tasks)()
        # check whether all tasks are still there
    ...
    def test_update_failed_dataset(self, mock_error):
        ...
        db.session.add_all([self.failed_task_dataset, self.bed, self.cooler, self.intervals])
        db.session.commit()
        # dispatch call
        add_app_context(self.app)(cleanup_failed_tasks)()
    ...
    def test_update_failed_collection(self, mock_error):
        ...
        # add stuff to database
        db.session.add_all([self.failed_task_collection, self.bed, self.collection, self.intervals])
        db.session.commit()
        # dispatch call
        add_app_context(self.app)(cleanup_failed_tasks)()
        # check whether task has been removed
