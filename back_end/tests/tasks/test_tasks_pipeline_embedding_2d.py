...
class TestEmbedding2DWorkerFunction(LoginTestCase, TempDirTestCase):
    ...
    def test_output_correct_shapes(self):
        ...
        # mock stuff
        array = np.stack(
            [np.random.normal(size=(10, 10)) for i in range(20)], axis=2
        )
        # make call
        embedding_results = _do_embedding_2d(
            array
        )
        self.assertEqual(embedding_results["embedding"].shape, (20, 2))
        self.assertEqual(embedding_results["clusters"]["large"]["cluster_ids"].shape, (20,))
        self.assertEqual(embedding_results["clusters"]["small"]["cluster_ids"].shape, (20,))
    ...
    def test_output_correct_shapes_if_feature_extraction_fails(self):
        ...
        # mock stuff
        array = np.stack(
            [np.full((10, 10),np.nan) for i in range(20)], axis=2
        )
        # make call
        embedding_results = _do_embedding_2d(
            array
