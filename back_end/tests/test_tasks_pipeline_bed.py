...
class TestPipelineBed(LoginTestCase, TempDirTestCase):
    ...
    def test_preprocess_bed_pipeline_step_called_correctly(self, mock_bed_pipeline_step,
                                                                 mock_set_progress,
                                                                 mock_sort_bed,
                                                                 mock_convert_bed,
        ...
        mock_sort_bed.assert_called_with(*["/test/path/test3.bed", "/test/path/test3_sorted.bed", self.app.config["CHROM_SIZES"]])
        ...
        for window in self.app.config["WINDOW_SIZES"]:
            ...
            mock_convert_bed.assert_any_call(*["/test/path/test3_sorted.bed", target_file, window])
