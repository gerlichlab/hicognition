...
class TestPipelineBed(LoginTestCase, TempDirTestCase):
    ...
    def test_helper_calls_dispatched_correctly_point_feature(
        ...
        for window in [size for size in self.app.config["PREPROCESSING_MAP"].keys() if size != "variable"]:
    ...
    def test_helper_calls_dispatched_correctly_interval_feature(
        ...
        pipeline_bed(2)
        # check whether dataset file_path is now sorted file_path
        self.assertEqual(self.dataset_interval.file_path, "/test/path/test4_cleaned.bed")
        # check whether sort_bed was called correctly
        mock_clean_bed.assert_called_with(
            *[
                "/test/path/test4.bed",
                "/test/path/test4_cleaned.bed",
            ]
        )
        # check whether convert_bed_to_bedpe and bed_pipeline_step was called correctly
...
class TestBedPreprocessPipelineStep(LoginTestCase, TempDirTestCase):
    ...
    def test_interval_features_add_correctly(
        self, mock_read_csv
    ):
    ...
    @patch("app.tasks.pd.read_csv")
    def test_point_features_add_correctly(
        self, mock_read_csv
    ):
    ...
    @patch("app.tasks.pd.read_csv")
    def test_whether_small_regions_are_not_downsampled(
        self, mock_read_csv
    ):
    ...
    @patch("app.tasks.pd.read_csv")
    def test_whether_large_regions_are_not_downsampled(
        self, mock_read_csv
    ):
